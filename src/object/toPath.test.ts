import { assertEquals, assertNotStrictEquals, assertStrictEquals } from "../test_deps.ts"
...
Deno.test("should not coerce symbols to strings", () => {
  ...
  for (const value of [symbol, object, [symbol], [object]]) {
    ...
    assertEquals(isSymbol(actual[0]), true, `symbol was not preserved: ${Deno.inspect(value)}`)
  }
})

Deno.test("should handle complex paths", () => {
  const actual = toPath('a[-1.23]["[\\"b\\"]"].c[\'[\\\'d\\\']\'][\ne\n][f].g');
  assertEquals(actual, ['a', '-1.23', '["b"]', 'c', "['d']", '\ne\n', 'f', 'g'])
})
