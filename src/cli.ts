...
function help() {
  ...
  ui.div({
    ...
    text: "Updates the `mod.ts` file by reading `**/*.ts` to determine what is exportable."
...
if (ARGS["update-mod"]) {
  ...
  let directories = Array.from(Deno.readDirSync(path.join("/"))).filter((entry) => entry.isDirectory )
  for (const entry of directories) {
    ...
    for (let entry of tsFiles) {
      ...
      const match = /\*\s+@export\s+[\s\S]*?(?=\n.*export function)\s*export\s+function\s+([^<(]+)/g.exec(contents)
      if (match) {
        modExports[moduleName][match[1] as string] = "./" + relative(srcPath.join("/"), path.join("/"))
      }
      path.pop()
    }
    path.pop()
  ...
  for (const modName of modNames) {
    ...
    for (const exportName of Object.keys(modExports[modName]).sort()) {
      modLines.push(`export { ${exportName} } from "${modExports[modName][exportName]}";`);
    }
  }
